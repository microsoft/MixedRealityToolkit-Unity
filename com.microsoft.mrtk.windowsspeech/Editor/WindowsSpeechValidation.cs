    ...
    internal static class WindowsSpeechValidation
    {
        ...
        private static BuildValidationRule GenerateMicrophoneCapabilityRule()
        {
            return new BuildValidationRule()
            {
                ...
                (MRTKProjectValidation.GetLoadedSubsystemsForBuildTarget(BuildTargetGroup.WSA)?.Contains(typeof(WindowsSpeechRecognitionSubsystem))).GetValueOrDefault(),
                Category = "MRTK3",
                Message = "WindowsPhrase/SpeechRecognitionSubsystem requires the WSA Microphone capability to be set to true.",
