...
{
    /// <summary>
    /// Augments the HandConstraint to also check if a given joint is facing the user before activation. This solver only works 
    ...
    /// </summary>
    public class HandConstraintJointUp : HandConstraint
    {
        [SerializeField]
        private TrackedHandJoint joint = TrackedHandJoint.Wrist;
        [Tooltip("Set to true if the joint should be facing away from the camera instead of toward")]
        public bool jointFacingAway = true;
        ...
        protected override bool IsHandActive(IMixedRealityController hand)
        {
            ...
            if (jointedHand != null)
            {
                if (jointedHand.TryGetJoint(joint, out jointPose))
                {
                    if (requireFlatHand)
                    {
                        ...
                        {
                            var handNormal = Vector3.Cross(indexTipPose.Position - jointPose.Position,
                            ...
                            if (Vector3.Angle(jointPose.Up, handNormal) > flatHandThreshold)
                            {
                                return false;
                            }
                        }
                ...
                return Vector3.Angle(jointPose.Up * (jointFacingAway ? -1f : 1f), CameraCache.Main.transform.forward) < facingThreshold;
            }
