...
{
    [AddComponentMenu("Scripts/MRTK/SDK/ConePointerVisual")]
    public class ConePointerVisual : MonoBehaviour
    {
        ...
        private void CheckInitialization()
        {
            if (pointer == null)
            {
                pointer = GetComponent<ConePointer>();
            }

            if (pointer == null)
            {
                Debug.LogError($"No ConePointer found on {gameObject.name}.");
            }
        ...
        private void CheckAsset(object asset, string fieldname)
        {
            if (asset == null)
            {
                Debug.LogError($"No {fieldname} specified on {gameObject.name}.ConePointerVisual. Did you forget to set the {fieldname}?");
            }
        }

        private static readonly ProfilerMarker UpdatePerfMarker = new ProfilerMarker("[MRTK] ConePointerVisual.Update");
        ...
        private static readonly ProfilerMarker GetGrabbedObjectPerfMarker = new ProfilerMarker("[MRTK] ConePointer.GetGrabbedObject");
