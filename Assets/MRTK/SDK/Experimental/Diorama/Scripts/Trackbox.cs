...
{
    [HelpURL("https://microsoft.github.io/MixedRealityToolkit-Unity/Documentation/README_Trackbox.html")]
    public class Trackbox : MonoBehaviour, IMixedRealityPointerHandler, IMixedRealityFocusChangedHandler
    {
        #region Public Enums
        ...
        [SerializeField]
        [Tooltip("Transform that will be dragged.")]
        ...
        [SerializeField]
        [Tooltip("Transform that will be used as an transform reference (the trackbox origin).")]
        private Transform referenceTransform = null;

        /// <summary>
        /// Transform that will be used as an transform reference (the trackbox origin).
        /// </summary>
        public Transform ReferenceTransform
        {
            get => referenceTransform;
        ...
        private TrackpadMoveLogic moveLogic;
        ...
        public ElasticExtentProperties<Vector3> extentProperties = new ElasticExtentProperties<Vector3>
        {
            ...
            MaxStretch = 0,
            SnapToEnds = false,
            SnapPoints = new Vector3[] { new Vector3(0.1f, 0.1f, 0.1f) } // Will be set to the specified snap interval at runtime
        };
        ...
        private void Awake()
        {
            moveLogic = new TrackpadMoveLogic();
        ...
        public void OnPointerDown(MixedRealityPointerEventData eventData)
        {
            ...
            if (pointerIdToPointerMap.Count > 0)
            {
                ...
                // as long as the Trackbox is active.
        ...
        private void HandleTwoHandManipulationUpdated()
        {
            ...
            if (twoHandedManipulationType.HasFlag(TransformFlags.Move))
            {
                ...
                //targetTransform.Position = moveLogic.Update(pose, targetTransform.Rotation, targetTransform.Scale, manipulationScale, true);
        ...
        private void HandleOneHandMoveStarted()
        {
            ...
            MixedRealityPose hostPose = new MixedRealityPose(HostTransform.position, HostTransform.rotation);
            moveLogic.Setup(pointerPose, pointerData.GrabPoint, hostPose);
        }
        ...
        private void HandleOneHandMoveUpdated()
        {
            ...
            targetTransform.Position = moveLogic.Update(pointerData.GrabPoint, manipulationScale);
        ...
        private void ApplyTargetTransform(MixedRealityTransform targetTransform)
        {
            if (rigidBody == null)
            {
                ...
                {
                    Vector3 localTarget;
                    if (HostTransform.parent != null)
                    {
                        localTarget = HostTransform.parent.InverseTransformPoint(targetTransform.Position);
                    }
                    else
                    {
                        localTarget = targetTransform.Position;
                    }
