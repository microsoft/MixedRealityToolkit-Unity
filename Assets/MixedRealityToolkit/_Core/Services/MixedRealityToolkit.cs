...
namespace Microsoft.MixedReality.Toolkit.Core.Services
{
    ...
    public class MixedRealityToolkit : MonoBehaviour
    {
        ...
        /// </summary>
        /// <remarks>
        /// NOTE: To avoid circular references in the editor, this method should not access the ActiveProfile property.
        /// </remarks>
        ...
        private void GetServices(Type type, string serviceName, ref List<IMixedRealityService> services)
        {
            if (type == null)
            {
                if ((ActiveProfile.LoggingLevel & LoggingLevels.Error) != 0)
                {
                    Debug.LogError("Unable to get services with a type of null.");
                }
                return;
            }
