...
namespace Microsoft.MixedReality.Toolkit.Internal.Devices.WindowsMixedReality
{
    ...
    public struct WindowsMixedRealityGestureController : IMixedRealityController
    {
        ...
        public WindowsMixedRealityGestureController(ControllerState controllerState, Handedness controllerHandedness, IMixedRealityInputSource inputSource, List<IInteractionMapping> interactions = null) : this()
        {
            ...
            for (uint i = 0; i < mappings.Length; i++)
            {
                ...
                switch (mappings[i].AxisType)
                {
                    case AxisType.Digital:
                        Interactions.Add(new InteractionMapping<bool>(i, mappings[i].AxisType, mappings[i].InputType, mappings[i].InputAction));
                        break;
                    case AxisType.SingleAxis:
                        Interactions.Add(new InteractionMapping<float>(i, mappings[i].AxisType, mappings[i].InputType, mappings[i].InputAction));
                        break;
                    case AxisType.DualAxis:
                        Interactions.Add(new InteractionMapping<Vector2>(i, mappings[i].AxisType, mappings[i].InputType, mappings[i].InputAction));
                        break;
                    case AxisType.ThreeDoFPosition:
                        Interactions.Add(new InteractionMapping<Vector3>(i, mappings[i].AxisType, mappings[i].InputType, mappings[i].InputAction));
                        break;
                    case AxisType.ThreeDoFRotation:
                        Interactions.Add(new InteractionMapping<Quaternion>(i, mappings[i].AxisType, mappings[i].InputType, mappings[i].InputAction));
                        break;
                    case AxisType.SixDoF:
                        Interactions.Add(new InteractionMapping<Tuple<Vector3, Quaternion>>(i, mappings[i].AxisType, mappings[i].InputType, mappings[i].InputAction));
                        break;
                    case AxisType.None:
                    case AxisType.Raw:
                        Interactions.Add(new InteractionMapping<object>(i, mappings[i].AxisType, mappings[i].InputType, mappings[i].InputAction));
                        break;
                    default:
