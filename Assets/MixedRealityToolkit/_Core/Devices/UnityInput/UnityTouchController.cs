    ...
    public class UnityTouchController : BaseController
    {
        ...
        public override void SetupDefaultInteractions(Handedness controllerHandedness)
        {
            ...
            if (MixedRealityOrchestrator.Instance.ActiveProfile.IsInputSystemEnabled &&
                MixedRealityOrchestrator.Instance.ActiveProfile.InputSystemProfile.GesturesProfile != null)
            {
                for (int i = 0; i < MixedRealityOrchestrator.Instance.ActiveProfile.InputSystemProfile.GesturesProfile.Gestures.Length; i++)
                {
                    var gesture = MixedRealityOrchestrator.Instance.ActiveProfile.InputSystemProfile.GesturesProfile.Gestures[i];
        ...
        {
            MixedRealityOrchestrator.InputSystem?.RaisePointerDown(InputSource.Pointers[0], Interactions[2].MixedRealityInputAction);
            isTouched = true;
            MixedRealityOrchestrator.InputSystem?.RaiseGestureStarted(this, holdingAction);
            isHolding = true;
        }
        ...
        public void Update()
        {
            ...
            if (TouchData.phase == TouchPhase.Moved)
            {
                ...
                {
                    MixedRealityOrchestrator.InputSystem?.RaisePositionInputChanged(InputSource, Interactions[0].MixedRealityInputAction, TouchData.deltaPosition);
                }

                lastPose.Position = InputSource.Pointers[0].BaseCursor.Position;
                lastPose.Rotation = InputSource.Pointers[0].BaseCursor.Rotation;
                MixedRealityOrchestrator.InputSystem?.RaiseSourcePoseChanged(InputSource, this, lastPose);

                Interactions[1].PoseData = lastPose;

                if (Interactions[1].Changed)
                {
                    MixedRealityOrchestrator.InputSystem?.RaisePoseInputChanged(InputSource, Interactions[1].MixedRealityInputAction, lastPose);
                }
                ...
                if (!isManipulating)
                {
                    ...
                    {
                        MixedRealityOrchestrator.InputSystem?.RaiseGestureCanceled(this, holdingAction);
                        isHolding = false;

                        MixedRealityOrchestrator.InputSystem?.RaiseGestureStarted(this, manipulationAction);
                        isManipulating = true;
                    }
                }
                else
                {
                    MixedRealityOrchestrator.InputSystem?.RaiseGestureUpdated(this, manipulationAction, TouchData.deltaPosition);
                }
            }
        }
        ...
        public void EndTouch()
        {
            if (TouchData.phase == TouchPhase.Ended)
            {
                if (Lifetime < K_CONTACT_EPSILON)
                {
                    ...
                    if (isManipulating)
                    {
                        MixedRealityOrchestrator.InputSystem?.RaiseGestureCanceled(this, manipulationAction);
                ...
                else if (Lifetime < MaxTapContactTime)
                {
                    ...
                    MixedRealityOrchestrator.InputSystem?.RaisePointerClicked(InputSource.Pointers[0], Interactions[2].MixedRealityInputAction, TouchData.tapCount);
                }

                if (isHolding)
                {
                    MixedRealityOrchestrator.InputSystem?.RaiseGestureCompleted(this, holdingAction);
                    isHolding = false;
                }

                if (isManipulating)
                {
                    MixedRealityOrchestrator.InputSystem?.RaiseGestureCompleted(this, manipulationAction, TouchData.deltaPosition);
                    isManipulating = false;
                }
            }
            ...
            MixedRealityOrchestrator.InputSystem?.RaisePointerUp(InputSource.Pointers[0], Interactions[2].MixedRealityInputAction);
