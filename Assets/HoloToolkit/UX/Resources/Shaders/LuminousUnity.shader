...
// sources (emission, reflection etc), the second pass captures the first 4 spot lights, implemented
// in Unity's fucked up approach to this.
//
// All computation based upon lighting is code-identical to the HLSL equivalent.
//
//
// To Do:
// - implement lights as parameters to the shader and wrap filling in the relevent values with a script
...
//
			...
			float4 frag(in Vs2Ps psIn) : SV_TARGET
			{
				...
				// due to the idiosyncracies of Unity at this point we sample the one light that is known to be calling
		...
		// and all light sources with cookies - ie spot lights - for these use cookies to achieve the correct angle
		Pass {
