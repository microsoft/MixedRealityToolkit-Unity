    ...
    public class UAudioProfiler : EditorWindow
    {
        ...
        private void CollectProfilerEvents(ProfilerEvent[] currentEvents)
        {
            ...
            for (int i = 0; i < currentEvents.Length; i++)
            {
                ...
                tempEvent.EmitterName = currentEvent.PrimarySource.gameObject.name;
                //Bus might be null, Unity defaults to Editor-hidden master bus
        ...
        private void OnGUI()
        {
            if (!EditorApplication.isPlaying)
            {
                GUILayout.Label("Profiler only active in play mode!");
                return;
            }
