...
namespace MixedRealityToolkit.UX.BoundingBoxes
{
    public class BoundingRig : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            // MRTK_FIX
            ApppBarPrefab.BoundingBox = boxInstance;
        ...
        {
            //Debug.Log("########################## FocusOnHandle " + handle.name);
        ...
        {
            Debug.Log("################ AppBarButton_OnButtonPressed ");
            if (showRig == true)
            {
                Deactivate();
            }
