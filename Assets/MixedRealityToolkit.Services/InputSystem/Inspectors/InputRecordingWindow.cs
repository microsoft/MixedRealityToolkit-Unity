...
{
    /// <summary>
    /// Tool for converting binary input animation data to/from AnimationClip
    /// </summary>
    public class InputAnimationConverter : EditorWindow
    {
        [MenuItem("Mixed Reality Toolkit/Utilities/Input Animation Converter")]
        private static void ShowWindow()
        {
            InputAnimationConverter window = GetWindow<InputAnimationConverter>();
            window.titleContent = new GUIContent("Input Animation Converter");
        ...
        private ToolMode mode = ToolMode.Import;
        private static string[] ToolModeNames = Enum.GetNames(typeof(ToolMode));
        ...
        void OnGUI()
        {
            mode = (ToolMode)GUILayout.SelectionGrid((int)mode, ToolModeNames, ToolModeNames.Length, "button");

            switch (mode)
            {
                case ToolMode.Import:
                    DrawImportGUI();
                    break;
        ...
        private void DrawImportGUI()
        {
            ...
            {
                filepath = EditorUtility.OpenFilePanel("Select input animation file", "", "bin");
            }
            EditorGUILayout.EndHorizontal();

            GUI.enabled = (filepath.Length > 0);
