...
SubShader {
  ...
  #define HUX_GEO_SET_EXTRA3(A) vxOut[vxOutCount].extra3=A;
        ...
        out float3 Distances_Sqrd    )
    {
        ...
        //Average = (VA + VB + VC)*float3(1.0/3.0,1.0/3.0,1.0/3.0);
    }
        ...
        out float4 Final_Color    )
    {
        ...
        Fill_Color.rgb = Fill_Color_In.rgb * fillProgress; // * sqrt(dot(d,d)) 
    ...
    void Edges_B126(
        ...
        out half inLine    )
    {
        float3 fw = fwidth(Edges.xyz); //*max(Edge_Width,1.0);
        // old version float3 a = smoothstep(float3(0.0,0.0,0.0),fw*Filter_Width,Edges.xyz);
