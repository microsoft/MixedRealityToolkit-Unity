...
{
    internal abstract class SynchronizedDynamicGameObjectHierarchy<TComponentService> : SynchronizedComponent<TComponentService, byte> where TComponentService : Singleton<TComponentService>, ISynchronizedComponentService
    {
        ...
        protected override void ProcessNewConnections(IEnumerable<SocketEndpoint> connectionsRequiringFullUpdate)
        {
            foreach (SocketEndpoint newConnection in connectionsRequiringFullUpdate)
            {
                if (DynamicObject == null)
                {
                    SynchronizedTransform.BlockedConnections.Add(newConnection);
                }
                else
                {
                    DynamicObject.GetComponent<SynchronizedTransform>().BlockedConnections.Add(newConnection);
                }
            }
        }
        ...
        private void OnDynamicObjectConstructed()
        {
            ...
            {
                if (synchronizedTransform.BlockedConnections.Remove(endpoint))
                {
                    DynamicObject.GetComponent<SynchronizedTransform>().BlockedConnections.Add(endpoint);
                }
                else
                {
        ...
        protected void SendInstantiationRequest()
        {
            ...
            foreach (KeyValuePair<SocketEndpoint, PerConnectionInstantiationState> state in perConnectionInstantiationState)
            {
                SynchronizedTransform.BlockedConnections.Add(state.Key);
        ...
        protected virtual void Read(SocketEndpoint sendingEndpoint, BinaryReader message, byte changeType)
        {
            switch (changeType)
            {
                ...
                case ChangeType.RemoteHierarchyBound:
                    if (DynamicObject != null)
                    {
                        DynamicObject.GetComponent<SynchronizedTransform>().BlockedConnections.Remove(sendingEndpoint);
                    }
                    break;
            }
        }
        ...
        private void SendTransformHierarchyBinding(SocketEndpoint sendingEndpoint)
        {
            ...
            using (BinaryWriter message = new BinaryWriter(memoryStream))
            {
                ...
                DynamicObject.GetComponent<SynchronizedTransform>().WriteChildHierarchyTree(message);
