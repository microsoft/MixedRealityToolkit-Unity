...
{
    internal class SynchronizedMaterials
    {
        ...
        public void UpdateMaterials(Renderer renderer, SynchronizationPerformanceParameters performanceParameters, Material[] materials, out bool areMaterialsDifferent)
        {
            UpdateCachedMaterials(materials, out areMaterialsDifferent);

            if (renderer != null && performanceParameters.MaterialPropertyBlocks == SynchronizationPerformanceParameters.FeatureInclusionType.SynchronizeFeature)
            {
                using (SynchronizationPerformanceMonitor.Instance.MeasureScope(SynchronizationPerformanceFeature.MaterialPropertyBlockUpdate))
                {
        ...
        public void SendMaterialPropertyChanges(IEnumerable<SocketEndpoint> endpoints, Renderer renderer, SynchronizationPerformanceParameters performanceParameters, Action<BinaryWriter> writeHeader, Func<MaterialPropertyAsset, bool> shouldSynchronizeMaterialProperty)
        {
            Renderer usedRenderer = performanceParameters.MaterialPropertyBlocks == SynchronizationPerformanceParameters.FeatureInclusionType.SynchronizeFeature ? renderer : null;
            using (SynchronizationPerformanceMonitor.Instance.MeasureScope(SynchronizationPerformanceFeature.MaterialPropertyUpdate))
            {
