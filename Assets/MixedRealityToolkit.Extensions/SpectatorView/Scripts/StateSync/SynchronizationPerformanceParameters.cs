...
{
    internal class SynchronizationPerformanceParameters : MonoBehaviour
    {
        ...
        [SerializeField]
        [Tooltip("Controls how frequently each GameObject checks for attached components that have a related SynchronizedComponent.")]
        protected PollingFrequency checkForSynchronizedComponents = PollingFrequency.UpdateContinuously;
        ...
        private SynchronizationPerformanceParameters parentParameters;
        ...
        private T GetInheritedProperty<T>(Func<SynchronizationPerformanceParameters, T> getter, T inhertedValue, T defaultValue)
        {
            SynchronizationPerformanceParameters parameters = this;
        ...
        public PollingFrequency CheckForSynchronizedComponents
        {
            get { return GetInheritedProperty(p => p.checkForSynchronizedComponents, PollingFrequency.InheritFromParent, PollingFrequency.UpdateContinuously); }
        }
        ...
        {
            if (GetComponent<DefaultSynchronizationPerformanceParameters>() != null)
            {
            ...
            else
            {
                ...
                {
                    parentParameters = DefaultSynchronizationPerformanceParameters.Instance;
                }
                else
                {
                    parentParameters = transform.parent.GetComponentInParent<SynchronizationPerformanceParameters>();
        ...
        public static SynchronizationPerformanceParameters CreateEmpty()
        {
            if (emptyParametersGameObject == null)
            {
                emptyParametersGameObject = new GameObject("EmptySychronizationPerformanceParameters");
            }
            return ComponentExtensions.EnsureComponent<SynchronizationPerformanceParameters>(emptyParametersGameObject);
        }
    }
}