...
namespace Interact.Controls
{
    ...
    public class SliderGestureControl : GestureInteractiveControl
    {
		...
		[Tooltip("Enable tapping on the bar to move the slider")]
		...
		/// <summary>
        /// Tap event, allows tapping on the bar to jump the slider to that location.
        /// </summary>
        public override void Tap()
        {
            ...
            if (EnableTap)
            {
                Vector3 localV = Vector3.zero;// TEMP SliderBar.transform.transform.InverseTransformPoint(GazeManager.Instance.HitPosition) - mStartCenter;
                float delta = localV.magnitude;

                if (Vector3.Dot(localV, mSliderVector) < 0)
                {
                    delta = -delta;
                }

                float percent = (delta + mSliderMagnitude * 0.5f) / mSliderMagnitude;
                
                GestureStarted = false;
                SetGestureValue(percent);
            }
        }
        ...
        public override void SetGestureValue(float gestureValue)
        {
