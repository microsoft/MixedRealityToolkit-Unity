...
{
    /// <summary>
    /// Tests for NewObjectManipulator
    /// </summary>
    public class NewObjectManipulatorTests : BaseRuntimeInputTests
    {

        /// <summary>
        /// Verifies that creating an NewObjectManipulator at runtime properly
        ...
        public IEnumerator TestNewObjectManipulatorInteractorRules()
        {
            ...
            objManip.DisableInteractorType(typeof(IGrabInteractor));
            yield return RuntimeTestUtilities.WaitForUpdates();

            yield return rightHand.SetHandshape(HandshapeId.Pinch);
            yield return RuntimeTestUtilities.WaitForUpdates();

            Assert.IsFalse(objManip.IsGrabSelected,
                "ObjManip was still grab selected after removing the grab interactor from the set of allowed interactors.");
            Assert.IsFalse(objManip.IsPokeSelected,
                "ObjManip was PokeSelected. Should not be possible.");
            ...
            objManip.DisableInteractorType(typeof(IGazeInteractor));
            yield return RuntimeTestUtilities.WaitForUpdates();
            yield return rightHand.SetHandshape(HandshapeId.Pinch);
            yield return RuntimeTestUtilities.WaitForUpdates();

            Assert.IsFalse(objManip.IsGazePinchSelected,
                "ObjManip was still gaze selected after removing the gaze interactor from the set of allowed interactors.");
        }
        ...
        /// <summary>
        /// Test creating adding a NewObjectManipulator to GameObject programmatically.
        /// Should be able to run scene without getting any exceptions.
        /// </summary>
        [UnityTest]
        public IEnumerator NewObjectManipulatorInstantiate()
        {
        ...
        /// <summary>
        /// Test creating NewObjectManipulator and receiving hover enter/exit events
        /// from gaze provider.
        /// </summary>
        [UnityTest]
        public IEnumerator NewObjectManipulatorGazeHover()
        {
            var testObject = GameObject.CreatePrimitive(PrimitiveType.Cube);
            testObject.transform.localScale = Vector3.one * 0.2f;
            ...
            NewObjectManipulator.hoverEntered.AddListener((eventData) => hoverEnterCount++);
            NewObjectManipulator.hoverExited.AddListener((eventData) => hoverExitCount++);
            ...
            Assert.AreEqual(1, hoverEnterCount, $"NewObjectManipulator did not receive hover enter event, count is {hoverEnterCount}");
            ...
            Assert.AreEqual(1, hoverExitCount, "NewObjectManipulator did not receive hover exit event");
            ...
            Assert.IsTrue(hoverExitCount == 1, "NewObjectManipulator received the second hover event");
        ...
        /// </summary>
        [UnityTest]
        public IEnumerator NewObjectManipulatorOneHandMoveNear()
        {
            ...
            foreach (var objectScale in objectScales)
            {
                ...
                foreach (NewObjectManipulator.RotateAnchorType rotationAnchorType in Enum.GetValues(typeof(NewObjectManipulator.RotateAnchorType)))
                {
                    NewObjectManipulator.RotationAnchorNear = rotationAnchorType;
                    ...
                    Assert.IsTrue(NewObjectManipulator.IsGrabSelected, $"NewObjectManipulator didn't get grabbed on pinch!");
                    ...
                    // if (rotationAnchorType == NewObjectManipulator.RotateAnchorType.RotateAboutObjectCenter)
                    // {
                    ...
                    for (int i = 1; i <= numCircleSteps; ++i)
                    {
                        ...
                        if (rotationAnchorType == NewObjectManipulator.RotateAnchorType.RotateAboutObjectCenter)
                        {
                            ...
                            Vector3 grabPoint = NewObjectManipulator.firstInteractorSelecting.GetAttachTransform(NewObjectManipulator).position;
                    ...
                    // make sure that the offset between grab and object centre hasn't changed while rotating
                    Vector3 currentGrabPoint = NewObjectManipulator.firstInteractorSelecting.GetAttachTransform(NewObjectManipulator).position;
                    ...
                    currentGrabPoint = NewObjectManipulator.firstInteractorSelecting.GetAttachTransform(NewObjectManipulator).position;
        ...
        /// </summary>
        [UnityTest]
        public IEnumerator NewObjectManipulatorOneHandMoveFar()
        {
            // Disable gaze interactions for this unit test;
            InputTestUtilities.DisableGaze();
            ...
            foreach (var objectScale in objectScales)
            {
                ...
                foreach (NewObjectManipulator.RotateAnchorType rotationAnchorType in Enum.GetValues(typeof(NewObjectManipulator.RotateAnchorType)))
                {
                    NewObjectManipulator.RotationAnchorFar = rotationAnchorType;
                    ...
                    Assert.IsTrue(NewObjectManipulator.isSelected, "NewObjectManipulator wasn't selected!");
                    ...
                    Vector3 initialGrabPoint = NewObjectManipulator.firstInteractorSelecting.GetAttachTransform(NewObjectManipulator).position;
                    Vector3 initialGrabPointInObject = testObject.transform.InverseTransformPoint(initialGrabPoint);
                    Vector3 initialGrabOffset = NewObjectManipulator.firstInteractorSelecting.transform.position - testObject.transform.position;
                    ...
                    for (int i = 1; i <= numCircleSteps; ++i)
                    {
                        ...
                        if (rotationAnchorType == NewObjectManipulator.RotateAnchorType.RotateAboutObjectCenter)
                        {
                            ...
                            Quaternion objectRotation = NewObjectManipulator.transform.rotation;
                            ...
                            Assert.IsTrue(NewObjectManipulator.firstInteractorSelecting.transform.InverseTransformPoint(NewObjectManipulator.transform.position).z > 0);
                        }
                        else
                        {
                    ...
                    Vector3 currentInteractorPosition = NewObjectManipulator.firstInteractorSelecting.transform.position;
                    ...
                    currentInteractorPosition = NewObjectManipulator.firstInteractorSelecting.transform.position;
        ...
        /// </summary>
        [UnityTest]
        public IEnumerator NewObjectManipulatorOneHandMoveGaze()
        {
            // Enable gaze interactions for this unit test;
            InputTestUtilities.EnableGaze();

            // Set up cube with NewObjectManipulator
            var testObject = GameObject.CreatePrimitive(PrimitiveType.Cube);
            Vector3 initialObjectPosition = InputTestUtilities.InFrontOfUser(1f);
            testObject.transform.position = initialObjectPosition;
            var NewObjectManipulator = testObject.AddComponent<NewObjectManipulator>();
            ...
            foreach (var objectScale in objectScales)
            {
                ...
                foreach (NewObjectManipulator.RotateAnchorType rotationAnchorType in Enum.GetValues(typeof(NewObjectManipulator.RotateAnchorType)))
                {
                    ...
                    // save relative pos grab point to object
                    // The firstInteractorSelecting is the one that is currently grabbing the object
                    Vector3 initialGrabPoint = NewObjectManipulator.firstInteractorSelecting.transform.position;
                    Vector3 initialGrabPointInObject = testObject.transform.InverseTransformPoint(initialGrabPoint);
                    Vector3 initialGrabOffset = NewObjectManipulator.firstInteractorSelecting.transform.position - testObject.transform.position;
                    ...
                    yield return hand.MoveTo(initialHandPosition + Vector3.forward);
                    // make sure that the offset between grab and object centre hasn't changed while rotating
                    Vector3 currentGrabPoint = NewObjectManipulator.firstInteractorSelecting.transform.position;
                    Vector3 currentOffset = currentGrabPoint - testObject.transform.position;
                    Assert.IsTrue(currentOffset.magnitude > initialGrabOffset.magnitude, $"Object did not move farther away when moving forward while doing gaze manipulation");

                    yield return hand.MoveTo(initialHandPosition + Vector3.back * 0.2f);
                    // make sure that the offset between grab and object centre hasn't changed while rotating
                    currentGrabPoint = NewObjectManipulator.firstInteractorSelecting.transform.position;
                    currentOffset = currentGrabPoint - testObject.transform.position;
                    Assert.IsTrue(currentOffset.magnitude < initialGrabOffset.magnitude, $"Object did not move closer when moving backwards while doing gaze manipulation");
        ...
        /// </summary>
        [UnityTest]
        public IEnumerator NewObjectManipulatorTwoHandedCentroid()
        {
            InputTestUtilities.DisableGaze();
            ...
            var NewObjectManipulator = testObject.AddComponent<NewObjectManipulator>();
            NewObjectManipulator.HostTransform = testObject.transform;
            NewObjectManipulator.SmoothingFar = false;
            NewObjectManipulator.SmoothingNear = false;
            // Configuring for two-handed interaction
            NewObjectManipulator.selectMode = UnityEngine.XR.Interaction.Toolkit.InteractableSelectMode.Multiple;
            ...
            NewObjectManipulator.AllowedManipulations = TransformFlags.Move | TransformFlags.Scale;

            // Only testing near manipulation
            NewObjectManipulator.AllowedInteractionTypes = InteractionFlags.Near;

            int manipulationStartedCount = 0;
            int manipulationEndedCount = 0;
            NewObjectManipulator.selectEntered.AddListener((med) => manipulationStartedCount++);
            NewObjectManipulator.selectExited.AddListener((med) => manipulationEndedCount++);
            ...
            // The NewObjectManipulator should recognize that we've begun manipulation.
            ...
            var leftGrabPoint = NewObjectManipulator.interactorsSelecting[0].transform.position;
            var rightGrabPoint = NewObjectManipulator.interactorsSelecting[1].transform.position;
            ...
            {
                yield return MoveHandsAndCheckCentroid(testCondition.Item1, testCondition.Item2, leftHand, rightHand, NewObjectManipulator, initialObjectPosition, originalCentroid, testObject.transform);
            }
        ...
        /// <summary>
        /// Helper function for NewObjectManipulatorTwoHandedCentroid. Will mirror desired handRotation
        ...
        */
        ...
        /// <summary>
        /// Test that objects with both NewObjectManipulator and Rigidbody respond
        /// correctly to static colliders.
        /// </summary>
        [UnityTest]
        public IEnumerator NewObjectManipulatorStaticCollision()
        {
            InputTestUtilities.InitializeCameraToOriginAndForward();
        ...
        /// </summary>
        [UnityTest]
        public IEnumerator NewObjectManipulatorRigidbodyCollision()
        {
            InputTestUtilities.InitializeCameraToOriginAndForward();
       ...
       /// </summary>
       [UnityTest]
       public IEnumerator NewObjectManipulatorNearThrow()
       {
           // set up cube with manipulation handler
           var testObject = GameObject.CreatePrimitive(PrimitiveType.Cube);
           testObject.transform.localScale = Vector3.one * 0.2f;
           ...
           // Assert.NotZero(rigidBody.angularVelocity.magnitude, "NewObjectManipulator should apply angular velocity to rigidBody upon release.");
           Assert.AreEqual(hand.GetVelocity(), rigidBody.velocity, "NewObjectManipulator should apply hand velocity to rigidBody upon release.");
       ...
       public IEnumerator NewObjectManipulatorFarThrow()
       {
           ...
           // Assert.IsTrue(rigidBody.velocity != hand.GetVelocity() && rigidBody.velocity.magnitude > 0.0f, "NewObjectManipulator should not dampen it's velocity to match the hand's upon release.");
           yield return RuntimeInputTestUtils.WaitForEnterKey();
           // This is just for debugging purposes, so object's movement after release can be seen.
           yield return hand.MoveTo(initialHandPosition);
           yield return hand.Hide();
